...
class PriceConverter {
  static String convertPrice(BuildContext context, double? price,
    ...
    return '${inRight ? '' : splashProvider.myCurrency!.symbol} ${(singleCurrency ? price : price! * splashProvider.myCurrency!.exchangeRate! * (1 / splashProvider.usdCurrency!.exchangeRate!))!.toStringAsFixed(splashProvider.configModel!.decimalPointSettings!).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')} ${inRight ? splashProvider.myCurrency!.symbol : ''}';
  }
  ...
  static String reverseConvertPriceWithoutSymbol(
      BuildContext context, double? localPrice,
      {double? discount,
      String? discountType,
      bool removeDecimalPoint = false}) {
    ...
    if (!singleCurrency &&
        splashProvider.myCurrency != null &&
        splashProvider.usdCurrency != null) {
      localPrice = localPrice /
          splashProvider.myCurrency!.exchangeRate! *
          splashProvider.usdCurrency!.exchangeRate!;
    }
