...
class RestockController with ChangeNotifier {
  ...
  Future<void> getRestockBrandList() async {
    ...
    ApiResponse apiResponse =
        await restockServiceInterface.getRestockBrandList();
  ...
  Future<void> deleteRestockListItem(int id, int index) async {
    _isLoading = true;
    ApiResponse apiResponse =
        await restockServiceInterface.deleteRestockItem(id);
  ...
  Future<void> getRestockProductList(int offset) async {
    if (offset == 1 &&
        (categoryId != null || (searchText != null || searchText != ''))) {
      restockProductModel = null;
      // notifyListeners();
    }
    ...
    _isLoading = true;
    ApiResponse apiResponse =
        await restockServiceInterface.getRestockProductList(data);
  ...
  void initVariationController(List<Variation> variations) {
    ...
    for (Variation v in variations) {
      variationQuantityController
          .add(TextEditingController(text: v.qty.toString()));
  ...
  Future<void> updateRestockProductQuantity(BuildContext context,
    ...
    ApiResponse apiResponse =
        await restockServiceInterface.updateRestockProductQuantity(
  ...
  void calculateVariationQuantity() {
    ...
    if (variationQuantityController.isNotEmpty) {
      ...
      for (int i = 0; i < variationQuantityController.length; i++) {
        variationTotalQuantity = variationTotalQuantity +
            (int.tryParse(variationQuantityController[i].text) ?? 0);
      }
    }
