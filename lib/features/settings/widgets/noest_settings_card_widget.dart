...
class _NoestSettingsCardWidgetState extends State<NoestSettingsCardWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    : (getTranslated('not_connected', context) ?? 'Not Connected'),
                ...
                Text(
                  getTranslated('use_your_vendor_guid_and_api_token_to_connect_your_noest_account_from_the_app', context)
                ...
                Wrap(
                  ...
                  children: shipProv.noestDeliveryMethods.map((method) {
                    return Container(
                      ...
                      child: Text(
                        getTranslated(method.title ?? method.name ?? '', context)
                        ...
                        onPressed: isBusy
                            ...
                            : () {
                                ...
                                if (noestGuid.isEmpty) {
                                  _showMessage(false, 'Please enter NOEST GUID');
                                  return;
                                }

                                if (apiToken.isEmpty) {
                                  _showMessage(false, 'Please enter NOEST API Token');
                                  return;
                                }
                                ...
                                shipProv.testNoestConnection(noestGuid, apiToken, (bool isSuccess, String message) {
                                  _showMessage(
                                    isSuccess,
                                    message.isNotEmpty ? message : getTranslated('noest_connection_test_completed', context) ?? 'NOEST connection test completed',
                                  );
                                });
                              },
                      ...
                      child: ElevatedButton(
                        onPressed: isBusy
                            ...
                            : () {
                                ...
                                if (noestGuid.isEmpty) {
                                  _showMessage(false, getTranslated('please_enter_noest_guid', context) ?? 'Please enter NOEST GUID');
                                  return;
                                }

                                if (apiToken.isEmpty) {
                                  _showMessage(false, getTranslated('please_enter_noest_api_token', context) ?? 'Please enter NOEST API Token');
                                  return;
                                }
                                ...
                                shipProv.saveNoestSettings(
                                  ...
                                  (bool isSuccess, String message) {
                                    _showMessage(
                                      isSuccess,
                                      message.isNotEmpty ? message : getTranslated('noest_settings_saved_successfully', context) ?? 'NOEST settings saved successfully',
                                    );
                                  },
                                );
                              },
