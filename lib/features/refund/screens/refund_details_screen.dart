...
class _RefundDetailsScreenState extends State<RefundDetailsScreen>
  ...
  void initState() {
    ...
    if (widget.fromNotification!) {
      ...
      Provider.of<RefundController>(context, listen: false)
          .getSingleRefundModel(Get.context!, widget.refundModel?.id);
    } else {
    ...
    return PopScope(
      ...
      child: Scaffold(
              ...
              child: AppBar(
                ...
                title: Consumer<RefundController>(
                  ...
                  return Column(children: [
                    Text(
                      '${refundModel?.orderId != null ? '${getTranslated('order', context)!}#' : ''} ${refundModel?.orderId ?? ' '}',
                      style: robotoMedium.copyWith(
                          color: Theme.of(context).textTheme.bodyLarge?.color),
                    ),
                    const SizedBox(height: Dimensions.paddingSizeExtraSmall),
                    Text(
                        refundModel?.createdAt != null
                ...
                actions: [
                  Consumer<RefundController>(builder: (context, refundReq, _) {
                    return InkWell(
                      onTap: refundModel?.order?.paymentMethod == null
                          ? null
                          : () {
          ...
          body: Consumer<RefundController>(
            ...
            if (widget.fromNotification! &&
                widget.refundModel?.status == null) {
              refundModel = refundController.refundModel;
            }
            return ((widget.fromNotification! &&
                        refundController.refundModel == null) ||
                    refundController.refundDetailsModel == null)
