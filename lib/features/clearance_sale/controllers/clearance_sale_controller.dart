...
class ClearanceSaleController extends ChangeNotifier {
  ...
  List<ClearanceSaleAddModel> get clearanceSaleAddModel =>
  ...
  ClearanceSaleProductModel? get clearanceSaleProductModel =>
      _clearanceSaleProductModel;
  ...
  Future<void> setSelectedOfferActiveType(int type,
  ...
  Future<void> getClearanceSaleProductList(int offset,
    ...
    ApiResponse apiResponse =
        await chatServiceInterface.getClearanceSaleProductList(offset);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (offset == 1) {
        _clearanceSaleProductModel = null;
        _clearanceSaleProductModel =
            ClearanceSaleProductModel.fromJson(apiResponse.response!.data);
      } else {
        _clearanceSaleProductModel!.totalSize =
            ClearanceSaleProductModel.fromJson(apiResponse.response!.data)
                .totalSize;
        _clearanceSaleProductModel!.offset =
            ClearanceSaleProductModel.fromJson(apiResponse.response!.data)
                .offset;
        _clearanceSaleProductModel!.products!.addAll(
            ClearanceSaleProductModel.fromJson(apiResponse.response!.data)
                .products!);
      }
    } else {
  ...
  Future<void> deleteClearanceSaleProductList(
      int? productId, int? index) async {
    _isLoading = true;
    ApiResponse apiResponse =
        await chatServiceInterface.clearanceSaleProductDelete(productId);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (index != null) {
        _clearanceSaleProductModel!.totalSize =
            _clearanceSaleProductModel!.totalSize! - 1;
  ...
  Future<void> deleteClearanceSaleAllProduct() async {
    ...
    notifyListeners();
    ApiResponse apiResponse =
        await chatServiceInterface.clearanceSaleProductDeleteAll();
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<void> updateClearanceSaleProductStatus(
      int productId, int isActive, int index) async {
    _isLoading = true;
    ApiResponse apiResponse = await chatServiceInterface
        .clearanceSaleProductStatusUpdate(productId, isActive);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _clearanceSaleProductModel!.products![index].isActive =
          (_clearanceSaleProductModel!.products![index].isActive == 0 ? 1 : 0);
  ...
  Future<void> getSearchProductList(String sellerId, int offset,
    ...
    ApiResponse apiResponse = await chatServiceInterface.getSellerProductList(
  ...
  Future<void> getClearanceConfigData() async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _clearanceConfigModel =
          ClearanceConfigModel.fromJson(apiResponse.response?.data);

      _isClearanceSaleActive = _clearanceConfigModel?.isActive == 1;
      _selectedDiscountType =
          _clearanceConfigModel?.discountType == 'product_wise' ? 2 : 1;
      discountController.text = '${_clearanceConfigModel?.discountAmount ?? 0}';
      _selectedOfferActiveType =
          _clearanceConfigModel?.offerActiveTime == 'specific_time' ? 2 : 1;

      _offerStartTime = _clearanceConfigModel!.offerActiveRangeStart != null
          ? formatTime(DateFormat('hh:mm:ss a')
              .parse(_clearanceConfigModel!.offerActiveRangeStart!))
          : null;

      _offerEndTime = _clearanceConfigModel!.offerActiveRangeEnd != null
          ? formatTime(DateFormat('hh:mm:ss a')
              .parse(_clearanceConfigModel!.offerActiveRangeEnd!))
          : null;

      _startDate = _clearanceConfigModel?.durationStartDate != null
          ? DateTime.parse(_clearanceConfigModel?.durationStartDate ?? '')
              .toLocal()
          : null;

      _endDate = _clearanceConfigModel?.durationEndDate != null
          ? DateTime.parse(_clearanceConfigModel?.durationEndDate ?? '')
              .toLocal()
          : null;
  ...
  Future<void> saveClearanceConfigData() async {
    ...
    Map<String, dynamic> data = {
      ...
      "offer_active_time":
          _selectedOfferActiveType == 1 ? 'always' : 'specific_time',
      'clearance_sale_duration':
          "${formatDurationDateTime(_startDate!)} - ${formatDurationDateTime(_endDate!)}"
    };

    if (_selectedOfferActiveType == 2) {
      data.addAll({"offer_active_range": '$_offerStartTime - $_offerEndTime'});
    }

    ApiResponse apiResponse =
        await chatServiceInterface.updateClearanceSaleConfigData(data);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      await getClearanceConfigData();
      showCustomSnackBarWidget(
          getTranslated('setup_updated_successfully', Get.context!),
          Get.context!,
          isError: false);
    } else {
      ApiChecker.checkApi(apiResponse);
  ...
  Future<void> updateConfigStatus(int value) async {
    _isLoading = true;
    ApiResponse apiResponse =
        await chatServiceInterface.updateConfigStatus(value);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  ({double minPrice, double maxPrice}) getMinMaxValues(
  ...
  void setSelectedProduct(Product product, int index) {
    ...
    _clearanceSaleAddModel.add(ClearanceSaleAddModel(
      id: product.id,
      type: 'percent',
      amount: 0,
      isWrongAmount: true,
      amountController: TextEditingController(
        text: _clearanceConfigModel?.discountType == 'flat'
            ? _clearanceConfigModel?.discountAmount.toString()
            : '0',
      ),
    ));
    notifyListeners();
  ...
  Future<void> clearanceSaleProductAdd() async {
    ...
    Map<String, dynamic> data = {"products": productList};

    ApiResponse apiResponse =
        await chatServiceInterface.clearanceSaleProductAdd(data);
  ...
  Future<void> clearanceSaleProductUpdate(
      int productId, String amount, String discountType) async {
    _isLoading = true;
    notifyListeners();
    ...
    ApiResponse apiResponse =
        await chatServiceInterface.updateClearanceSaleProductDiscount(data);
    ...
    if (apiResponse.response != null &&
      ...
      await getClearanceSaleProductList(1, reload: false);
      showCustomSnackBarWidget(
          getTranslated('product_discount_updated', Get.context!), Get.context!,
          isError: false);
    } else {
      ApiChecker.checkApi(apiResponse);
    }
