...
class ShopController extends ChangeNotifier {
  ...
  PaymentInformationModel? get paymentInformationModel =>
  ...
  Future<void> shopVacation(
      BuildContext context, VacationModel vacationModel) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse =
        await shopServiceInterface.vacation(vacationModel);
  ...
  void selectDateTime(String type, bool is24Hour, BuildContext context,
  ...
  Future<ResponseModel> getPaymentWithdrawalMethodList() async {
    ResponseModel responseModel;
    ApiResponse apiResponse =
        await shopServiceInterface.getPaymentWithdrawalMethodList();

    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _withdrawalMethodModel = null;
      _withdrawalMethodModel =
          WithdrawalMethodModel.fromJson(apiResponse.response!.data);
      responseModel = ResponseModel(true, 'successful');
    } else {
      String? errorMessage;
      if (apiResponse.error is String) {
  ...
  Future<ApiResponse> addPaymentInfo(
      BuildContext context, WithdrawAddModel vacationModel,
      {bool isUpdate = false}) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse =
        await shopServiceInterface.addPaymentInfo(vacationModel, isUpdate);
    if (apiResponse.response!.statusCode == 200) {
      _isLoading = false;
      if (!isUpdate) {
        showCustomSnackBarWidget(
            getTranslated('payment_method_added_successfully', Get.context!),
            Get.context!,
            isToaster: true,
            isError: false);
      } else {
  ...
  Future<void> getPaymentInfoList(int? offset) async {
    ApiResponse apiResponse =
        await shopServiceInterface.getPaymentInfoList(offset);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (offset == 1) {
        _paymentInformationModel =
        ...
        _paymentInformationModel!.data!.addAll(
            PaymentInformationModel.fromJson(apiResponse.response?.data).data ??
                []);
        _paymentInformationModel!.offset =
            PaymentInformationModel.fromJson(apiResponse.response?.data).offset;
        _paymentInformationModel!.totalSize =
            PaymentInformationModel.fromJson(apiResponse.response?.data)
                .totalSize;
        _paymentInformationModel!.limit =
            PaymentInformationModel.fromJson(apiResponse.response?.data).limit;
      }
    } else {
      ApiChecker.checkApi(apiResponse);
    }
  ...
  Future<void> updatePaymentMethodStatus(
    bool status,
    int index,
    int id,
  ) async {
    ApiResponse apiResponse =
        await shopServiceInterface.updateConfigStatus(status, id);
    if (apiResponse.response!.statusCode == 200) {
      showCustomSnackBarWidget(
          getTranslated('status_updated_successfully', Get.context!),
          Get.context!,
  ...
  Future<void> deletePaymentMethodStatus(int id, int index) async {
    ApiResponse apiResponse =
        await shopServiceInterface.deletePaymentMethod(id);
    if (apiResponse.response!.statusCode == 200) {
      showCustomSnackBarWidget(
          getTranslated('payment_method_deleted_successfully', Get.context!),
          Get.context!,
          isError: false,
          sanckBarType: SnackBarType.success);
  ...
  Future<void> setDefaultPaymentMethod(int id) async {
    ApiResponse apiResponse =
        await shopServiceInterface.setDefaultPaymentMethod(id);
    if (apiResponse.response!.statusCode == 200) {
      showCustomSnackBarWidget(
          getTranslated('payment_method_deleted_successfully', Get.context!),
          Get.context!,
  ...
  Future<void> updateSetupGuideApp(String key, int value) async {
    ApiResponse apiResponse =
        await shopServiceInterface.updateSetupGuideApp(key, value);
    if (apiResponse.response!.statusCode == 200) {
      getShopInfo();
    }
    notifyListeners();
  ...
  void previewDownload(
    ...
    if (true) {
      HttpClientResponse apiResponse =
          await shopServiceInterface.downloadTinCertificate(url);
      if (apiResponse.statusCode == 200) {
        ...
        showCustomSnackBarWidget(
            getTranslated('file_downloaded_successfully', Get.context!),
            Get.context!,
            isError: false);
        ...
        showCustomSnackBarWidget(
            getTranslated('file_download_failed', Get.context!), Get.context!);
