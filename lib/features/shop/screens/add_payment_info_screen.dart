...
class AddPaymentInfoScreen extends StatefulWidget {
  ...
  const AddPaymentInfoScreen(
      {super.key, this.formUpdate = false, this.paymentInfoData});
...
class _AddPaymentInfoScreenState extends State<AddPaymentInfoScreen> {
  ...
  void initState() {
    ...
    if (widget.formUpdate && widget.paymentInfoData != null) {
      ...
      shopController.setPaymentStatus(widget.paymentInfoData?.isActive ?? false,
          isUpdate: false);
      shopController.setSelectedPaymentMethod(
          widget.paymentInfoData?.withdrawMethod?.methodName ?? '');
      ...
      if (methodField.inputType == 'date') {
        ...
        (widget.paymentInfoData?.methodInfo ?? {}).forEach((key, value) {
          if (key == methodField.inputName) {
            ...
            methodField.textEditingController?.text =
                CountryCodeHelper.extractPhoneNumber(countryCode, value);
          }
        });
      } else {
        (widget.paymentInfoData?.methodInfo ?? {}).forEach((key, value) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBarWidget(
          title: getTranslated('add_new_payment_info', context)),
      body: Consumer<ShopController>(builder: (context, shopInfo, child) {
        paymentMethodList = [];
        paymentMethodList.addAll(shopInfo.withdrawalMethodModel!.data!
                          ...
                          children: [
                            ...
                            DropdownDecoratorWidget(
                              child: DropdownButton<String>(
                                ...
                                hint: Text(
                                  shopInfo.selectedPaymentMethod!.isNotEmpty
                            ...
                            ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                if (!widget.formUpdate &&
                                    shopInfo.paymentMethodFields[index]
                                            .inputType ==
                                        'phone' &&
                                    shopInfo.paymentMethodFields[index]
                                            .countryCode ==
                                        null) {
                                  shopInfo.setMethodCountryCode(
                                      index,
                                ...
                                return shopInfo.paymentMethodFields[index]
                                            .inputType !=
                                        'date'
                                    ? CustomTextFieldWidget(
                                        formProduct: true,
                                        ...
                                        border: true,
                                        controller: shopInfo
                                            .paymentMethodFields[index]
                                            .textEditingController,
                                        isPhoneNumber: shopInfo
                                                .paymentMethodFields[index]
                                                .inputType ==
                                            'phone',
                                        countryDialCode:
                                            CountryCode.fromCountryCode(Provider
                                                            .of<SplashController>(
                                                                context,
                                                                listen: false)
                                                        .configModel!
                                                    ...
                                                    .dialCode ??
                                                '+880',
                                        hintText: formatField(shopInfo
                                                .paymentMethodFields[index]
                                                .inputName ??
                                            ''),
                                        showCodePicker: shopInfo
                                                .paymentMethodFields[index]
                                                .inputType ==
                                            'phone',
                                        onCountryChanged:
                                                  ...
                                                  child: Padding(
                                                      ...
                                                      child: Text(
                                                        shopInfo
                                                                ...
                                                                null
                                                            ? DateConverter
                                                                .stringToLocalDateOnly(shopInfo
                                                                    .paymentMethodFields[
                                                                        index]
                                                                    .dateTime!
                                                                    .toString())
                          ...
                          child: shopProvider.isLoading
                              ...
                              : CustomButtonWidget(
                                  ...
                                  onTap: () {
                                    ...
                                    for (MethodFields methodField
                                        in shopProvider.paymentMethodFields) {
                                      if (methodField.inputType == 'date') {
                                        ...
                                        methodInfo.addAll({
                                          methodField.inputName ?? '':
                                              methodField.dateTime
                                                  .toString()
                                                  .split(' ')
                                                  .first
                                        });
                                      } else if (methodField.inputType ==
                                          'phone') {
                                        if ((methodField.textEditingController
                                                        ?.text ??
                                                    '')
                                                .isEmpty ||
                                            (methodField.countryCode ?? '')
                                                .isEmpty) {
                                          isMethodFieldEmpty = true;
                                          break;
                                        }
                                        ...
                                        methodInfo.addAll({
                                          methodField.inputName ?? '':
                                              methodField.textEditingController
                                                      ?.text ??
                                                  ''
                                        });
                                      }
                                    ...
                                    if (_methodNameController.text
                                        .trim()
                                        .isEmpty) {
                                      showCustomSnackBarWidget(
                                          getTranslated(
                                              'method_mame_is_empty', context),
                                          context,
                                          isError: false,
                                      ...
                                      showCustomSnackBarWidget(
                                          getTranslated(
                                              'check_all_method_field_is_empty',
                                              context),
                                          context,
                                          isError: false,
                                          sanckBarType: SnackBarType.warning);
                                    } else {
                                      WithdrawAddModel withdrawAddModel =
                                          WithdrawAddModel(
                                              withdrawMethodId: shopProvider
                                                  .getPaymentMethodId(shopProvider
                                                          .selectedPaymentMethod ??
                                                      ''),
                                              methodName:
                                                  _methodNameController.text,
                                              isActive: (shopProvider
