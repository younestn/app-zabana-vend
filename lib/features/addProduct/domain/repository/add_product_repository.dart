...
class AddProductRepository implements AddProductRepositoryInterface {
  ...
  Future<Map<String, dynamic>> _prepareRequestData({
    ...
    if (!(product.productType == 'digital' ||
        (addProduct.colorCodeList != null &&
            addProduct.colorCodeList!.isEmpty))) {
  ...
  List<Map<String, dynamic>> _prepareColorImages(
      List<ColorImage> colorImageObject) {
    return colorImageObject
        .where((image) =>
            image.imageName?.key != 'null' && image.imageName?.key != null)
        .map((image) => {
              'color': image.color,
  ...
  void _addBasicProductFields(
    ...
    fields.addAll({
      ...
      'publishing_house':
          jsonEncode(digitalVariationModel?.publishingHouse ?? []),
  ...
  void _addCategoryFields(
      Map<String, dynamic> fields, List<CategoryIds> categoryIds) {
  ...
  void _addDigitalVariationFields(Map<String, dynamic> fields,
      DigitalVariationModel digitalVariationModel) {
    fields.addAll({
  ...
  Future<List<MultipartWithKey>> processItems(
    ...
    if (digitalVariationModel?.digitalProductPreview != null) {
      ...
      multipartBody.add(
          MultipartWithKey(key: 'preview_file', multipartFile: multiPartFile));
    }
  ...
  Future<ApiResponse> deleteProductPreview(int? id) async {
    try {
      final response = await dioClient!
          .get("${AppConstants.deleteProductPreview}?product_id=$id");
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> getPublishingHouse() async {
    try {
      final response =
          await dioClient!.get(AppConstants.digitalPublishingHouse);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
