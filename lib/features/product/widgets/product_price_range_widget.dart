...
class ProductPriceRangeWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double maxPriceRange = PriceConverter.convertAmount(
        configModel?.productMaxPriceRange ?? 0, context);
        ...
        Row(
          ...
          children: [
            Flexible(
              child: Container(
                padding: const EdgeInsets.symmetric(
                    horizontal: Dimensions.iconSizeDefault),
                child: Column(
                      ...
                      CustomTextFieldWidget(
                        ...
                        onChanged: (String text) {
                          ...
                          if (tempMinPrice <= tempMaxPrice &&
                              tempMinPrice <= maxPriceRange &&
                              tempMaxPrice <= maxPriceRange) {
                            productController.setPriceRange(
                                tempMinPrice, tempMaxPrice);
                          } else {
                            productController.setPriceRangeValidity(
...
class _PointyTooltipPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Path path = Path()
      ...
      ..moveTo(
          size.width / 2 - triangleWidth / 2, size.height) // Left triangle edge
      ..lineTo(size.width / 2, size.height + triangleHeight) // Triangle point
      ..lineTo(size.width / 2 + triangleWidth / 2,
          size.height) // Right triangle edge
